...
export default class BarreEvenements extends Component {
  ...
  async getEvenementsForMonth(mm) {
    try {
      this.setState({
        evenementMensuel: this.trierEvenement(
          HelperEvenement.getEvenementsMonth(mm)
        ),
      });
    } catch (err) {
      console.log(err);
  ...
  trierEvenement(evenements) {
    return evenements.sort((a, b) =>
      a.date.jour > b.date.jour ? 1 : b.date.jour > a.date.jour ? -1 : 0
    );
  }
        ...
        <Layout>
          {this.state.device === "MOBILE" ? (
            <div className="contentPrésentationMobileF">
              <div>
                <h2 className="separator" style={{ fontWeight: "bold" }}>
                  CALENDRIER DU MOIS
                </h2>
              ...
              <Row>
                {this.state.evenementMensuel.length !== 0 ? (
                  <>
                    <Col>
                      <div>
                        <h5 style={{ color: "#021930", fontWeight: "bold" }}>
                          En {this.state.moisLettre}
                        </h5>
